...
FastAPI backend that combines procurement data, financial records, satellite imagery, and ML risk scoring to detect ghost infrastructure projects. Includes the single-project investigation pipeline (Sprints 1–5) for targeted on-demand auditing of individual projects.
...
│   └── versions/             # 10 migration files (initial → investigation FK)
...
│   │   └── investigation.py  # investigations — single-project audit state
...
│   │   └── investigations.py # Single-project investigation pipeline
...
### Investigations (Single-Project Pipeline)

| Method  | Path                                  | Description                                           |
| ------- | ------------------------------------- | ----------------------------------------------------- |
| `POST`  | `/api/v1/investigations`              | Create investigation (rate-limited: 20/hour)          |
| `POST`  | `/api/v1/investigations/{id}/enrich`  | Enrich with Perplexity AI context                     |
| `PATCH` | `/api/v1/investigations/{id}/context` | Manual context correction / fallback fill             |
| `POST`  | `/api/v1/investigations/{id}/scrape`  | Trigger full 7-stage pipeline (rate-limited: 10/hour) |
| `GET`   | `/api/v1/investigations/{id}/status`  | Poll stage progress                                   |
| `GET`   | `/api/v1/investigations/{id}/report`  | Full investigation report                             |

## Database Schema

Six core tables + investigations table with PostGIS geometry columns:
...
| `investigations`      | Single-project audit state — context JSONB, stage_statuses JSONB, project_uuid FK |
...
| `FinancialService`            | COB PDF ingestion (standard + Vision), absorption gap calculation         |
...
| `PerplexityEnrichmentService` | Perplexity Sonar-Pro API → structured `ProjectContext` for investigations |
...
| `ingest_cob_report_intelligent_task` | POST /investigations/{id}/scrape | Two-stage Vision COB extraction for investigations |
...
| `investigate_project_task`           | POST /investigations/{id}/scrape | 7-stage single-project investigation pipeline      |
...
| `PERPLEXITY_API_KEY`    | —                                        | Perplexity Sonar-Pro API key (investigation enrichment) |
| `PERPLEXITY_MODEL`      | `sonar-pro`                              | Perplexity model variant                                |
| `OPENAI_API_KEY`        | —                                        | OpenAI API key (IntelligentCoBParser Vision extraction) |
| `OPENAI_VISION_MODEL`   | `gpt-4o`                                 | GPT model for COB Vision calls                          |
| `VISION_MAX_PAGES`      | `20`                                     | Max candidate pages sent to Vision per PDF              |
| `VISION_DPI`            | `200`                                    | Page render DPI for Vision input                        |
| `VISION_COST_LIMIT_USD` | `5.0`                                    | Per-investigation OpenAI Vision spend cap               |
...
# Run all tests (334 pass, 82% coverage)
